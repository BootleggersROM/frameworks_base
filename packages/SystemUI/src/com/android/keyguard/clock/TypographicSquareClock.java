...
import com.android.keyguard.clock.extra.PaddingBackgroundColorSpan;
 ...
 */
public class TypographicSquareClock extends TextView {
    ...
    public TypographicSquareClock(Context context) {
        this(context, null);
    }

    public TypographicSquareClock(Context context, AttributeSet attrs) {
        this(context, attrs, 0);
    }

    public TypographicSquareClock(Context context, AttributeSet attrs, int defStyleAttr) {
        super(context, attrs, defStyleAttr);
        mDescFormat = ((SimpleDateFormat) DateFormat.getTimeFormat(context)).toLocalizedPattern();
        mDateFormat = ((SimpleDateFormat) DateFormat.getDateFormat(context)).toLocalizedPattern();
        ...
        mPadding = mResources.getDimensionPixelSize(R.dimen.typeClockBoxPadding);
    }
    ...
    public void onTimeChanged() {
        ...
        CharSequence formattedDate = DateFormat.format(mDateFormat, mTime);
        ...
        SpannedString typeTemplate = (SpannedString) mResources.getQuantityText(
                R.plurals.type_square_clock_header, hour);

        //Spanned typeTemplate = (Spanned) mResources.getString(R.string.clock_typeface_template);

        SpannableString todate = new SpannableString(formattedDate);
        todate.setSpan(new RelativeSizeSpan(0.64f), 0,todate.length(), 0); // set size
        ...
        for (int i = 0; i < annotations.length; i++) {
            ...
            if (ANNOTATION_COLOR.equals(key)) {
                spanType.setSpan(new PaddingBackgroundColorSpan(mAccentColor, mPadding),
                        spanType.getSpanStart(annotation), spanType.getSpanEnd(annotation),
                        Spannable.SPAN_EXCLUSIVE_EXCLUSIVE);
            }
        }

        setText(TextUtils.expandTemplate(spanType, mHours[hour], mMinutes[minute], todate));
    }
